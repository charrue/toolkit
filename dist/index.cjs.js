...
const count = (arr, callbackfn, ctx) => {
  let num = 0;
...
const findIndexRight = (arr, callbackfn, thisArg) => {
  const l = arr.length;
  for (let i = l - 1; i >= 0; i--) {
...
const findRight = (arr, callbackfn, thisArg) => {
...
function hexRgb(rawHex, options = {}) {
  if (typeof rawHex !== "string" || nonHexChars.test(rawHex) || !validHexSize.test(rawHex)) {
...
const colorizeMessage = (message, type) => {
...
function compose(...funcs) {
  ...
  return (...args) => rest.reduce((composed, f) => f(composed), first(...args));
}
...
function omit(data, attrs = []) {
  const result = __spreadValues({}, data);
  attrs.forEach((attr) => {
    delete result[attr];
  });
  return result;
...
const isHTMLElement = (obj) => (obj == null ? void 0 : obj.nodeName) || (obj == null ? void 0 : obj.tagName);
const isFn = isType([
  "Function",
  "AsyncFunction",
  "GeneratorFunction"
...
const isDef = (val) => !isNull(val) && !isUndefined(val);
const isUnDef = (val) => isNull(val) || isUndefined(val);
...
function createLogger(level = "info", loggerOptions = {}) {
  ...
  function output(type, msg, outputOptions = {}) {
    if (thresh >= LogLevels[type]) {
      ...
      const format = () => {
        ...
        return colorizeMessage(msg, outputOptions.color ? type : void 0);
      };
      if (allowClearScreen) {
...
const downloadFile = (url, filename, { beforeDownload, onDownload } = {}) => {
  if (!inBrowser) {
...
class SinglyLinkedList {
  ...
  removeAt(_index) {
    var _a;
    const index = _index >= 0 ? _index : this.size + _index;
    if (!this._head || this.size <= 0 || index > this.size - 1)
      return void 0;
    if (index === 0) {
    ...
    this._size--;
    return (_a = currentNode.next) == null ? void 0 : _a.value;
  }
  remove(value) {
  ...
  at(_index) {
    ...
    return currentNode == null ? void 0 : currentNode.value;
  }
  forEach(callback) {
    let current = this._head;
    let index = 0;
...
const padStart = (string, length, pad = "0") => {
  const s = `${string}`;
...
const padEnd = (string, length, pad = "0") => {
  const s = `${string}`;
  if (!s || s.length >= length)
    return string;
  const padChar = pad.length <= 1 ? pad : pad.charAt(0);
...
const parseSeconds = (milliseconds) => {
  return {
...
const formatSeconds = (formatter, timeData) => {
  const {
    days,
    hours,
    minutes,
...
function sleep(ms, callback) {
  return new Promise((resolve) => setTimeout(() => __async(this, null, function* () {
