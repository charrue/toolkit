/* eslint-disable */
// fork from https://github.com/unjs/perfect-debounce/blob/main/src/index.ts
...
async function applyPromised(fn: Function, _this: unknown, args: any[]) {
  const res = await fn.apply(_this, args);
  return res;
}
...
export function debounce <ArgumentsT extends unknown[], ReturnT>(
  ...
  const applyFn = async (scope: any, args: any[]) => {
    currentPromise = applyPromised(fn, scope, args);
    currentPromise
      .finally(() => {
        currentPromise = null;
        if (options.trailing && trailingArgs && !timeout) {
          const promise = applyFn(scope, trailingArgs);
          trailingArgs = null;
          return promise;
        }
      });
