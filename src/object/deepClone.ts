...
export const deepClone = <T>(raw: T): T => {
  ...
  const baseClone = <T>(raw: T, proto = false): T => {
    ...
    if (raw instanceof Map) {
      return new Map(baseClone(Array.from(raw))) as T
    }

    if (raw instanceof WeakMap) {
      return new WeakMap(
        baseClone(
          Array.from<any>(raw as any)
        )
      ) as T
    }

    if (raw instanceof Set) {
      return new Set(baseClone(Array.from(raw))) as T;
    }
