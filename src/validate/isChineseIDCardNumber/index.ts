// fork from https://github.com/fjc0k/vtils/blob/master/src/utils/isChineseIDCardNumber.ts
...
export function isChineseIDCardNumber(value: string): boolean {
  ...
  if (
    !isValidDate(
      Number(value.slice(6, 4)),
      Number(value.slice(10, 2)),
      Number(value.slice(12, 2)),
    )
  ) {
    return false;
  }
